...
if __name__ == '__main__':
    ...
    for source_number in np.arange(2, 5, 1):
        ...
        for test_i in np.arange(1, 2, 1):
            ...
            # Time and accuracy of MeICA_dev
            # 添加对meica_dev时间的测量
            pyfbss_tb.timer_start()
            hat_S = pyfbss.meica_dev(X, max_iter=100, tol=1e-04, break_coef=0.92, ext_multi_ica=extraction_base)
            time = pyfbss_tb.timer_value()
            Eval_dB = pyfbss_tb.bss_evaluation(S, hat_S, eval_type)
            tmp_meicadev.extend([Eval_dB, time])
            print('MeICA_dev:   ', Eval_dB, '; ', time)
        ...
        #添加
        Eve_MeICA_dev.append(tmp_meicadev)
    ...
    #添加
    save_data_csv(Eve_MeICA_dev, 'test_results/google_dataset/meica_dev.csv')
